...
class GrokVideoPlugin(Star):
    ...
    def _resolve_url(self, url: Optional[str]) -> Optional[str]:
        ...
        if not self._is_valid_video_url(url):
            logger.warning(f"URL: {url} 未通过有效性检查，协议可能缺失或不正确。")
            return None
